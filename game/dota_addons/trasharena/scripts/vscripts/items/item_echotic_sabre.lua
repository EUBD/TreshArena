...
function modifier_echotic_sabre_passive:DeclareFunctions(  )
	local hFuncs = { MODIFIER_PROPERTY_STATS_INTELLECT_BONUS,MODIFIER_PROPERTY_STATS_STRENGTH_BONUS,MODIFIER_PROPERTY_DAMAGEOUTGOING_PERCENTAGE,MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT,MODIFIER_PROPERTY_PREATTACK_BONUS_DAMAGE,MODIFIER_EVENT_ON_ATTACK_LANDED }
	return hFuncs
end

function modifier_echotic_sabre_passive:OnAttackLanded( params )
	...
	if params.attacker == caster then
		...
		if RollPercentage(chance) then
			if not caster.performFlag and not caster.Cooldown_use then
				caster.performFlag = true
...
function modifier_echotic_sabre_atk:DeclareFunctions(  )
	local hFuncs = { MODIFIER_PROPERTY_BASE_ATTACK_TIME_CONSTANT,MODIFIER_EVENT_ON_ATTACK_LANDED }
	return hFuncs
end
...
function modifier_echotic_sabre_atk:OnAttackLanded( params )
	...
	caster:RemoveModifierByName("modifier_echotic_sabre_atk")
end

function modifier_echotic_sabre_atk:GetModifierBaseAttackTimeConstant(  )
	return 0.01
end
