...
function modifier_item_bfury_datadriven_on_created(keys)
	if not keys.caster:IsRangedAttacker() then
		keys.ability:ApplyDataDrivenModifier(keys.caster, keys.caster, "modifier_war_axe_cleave", {duration = -1})
	end
end
...
function modifier_item_bfury_datadriven_on_destroy(keys)
	if not keys.caster:IsRangedAttacker() then
		keys.caster:RemoveModifierByName("modifier_war_axe_cleave")
	end
end
...
function modifier_item_bfury_datadriven_on_interval_think(keys)
	if not keys.caster:IsRangedAttacker() and not keys.caster:HasModifier("modifier_war_axe_cleave") then
		for i=0, 5, 1 do
			local current_item = keys.caster:GetItemInSlot(i)
			if current_item ~= nil then
				if current_item:GetName() == "item_war_axe" then
					keys.ability:ApplyDataDrivenModifier(keys.caster, keys.caster, "modifier_war_axe_cleave", {duration = -1})
				end
			end
		end
...
function modifier_item_bfury_datadriven_cleave_on_interval_think(keys)
	if keys.caster:IsRangedAttacker() then
		while keys.caster:HasModifier("modifier_war_axe_cleave") do
			keys.caster:RemoveModifierByName("modifier_war_axe_cleave")
		end
	end
end