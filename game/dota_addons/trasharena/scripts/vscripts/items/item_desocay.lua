...
function modifier_desocay_passive:OnAttackLanded( params )
	if IsServer() then
		...
		if caster == params.attacker and not target:IsMagicImmune() then
			local ability = self:GetAbility()
			local spa = ability:GetSpecialValueFor("spa") / 100
			local spab = ability:GetSpecialValueFor("spab") / 100
			local min_cap = ability:GetSpecialValueFor("min_cap")
			local name = target:GetUnitName()
			local armor_cap = target:GetPhysicalArmorBaseValue() * spa

			if armor_cap <= 0 then armor_cap = 0 end
			cap = math.floor(armor_cap)

			if cap <= 6 then cap = min_cap end
			ability:SetCurrentCharges(cap)
			...
			if target:HasModifier("modifier_desocay_disarmor") then
				...
				if self.stacks < ability:GetCurrentCharges() then
					target:SetModifierStackCount("modifier_desocay_disarmor", caster, self.stacks + 1)
				end

				if self.stacks >= ability:GetCurrentCharges() then target:SetModifierStackCount("modifier_desocay_disarmor", caster, ability:GetCurrentCharges()) end
			end
